<template>
    <div class="form-inline form-group mt-1">
        ...
        <div class="col-md-4">
            <select class="form-control" name="subcategory_id" v-model="subcategory" @change="getSubsubcategories">
                <option value="">Choisir la sous-catégorie</option>
                <option v-for="data in subcategories" :value="data.id" :key="data.id">{{data['subcategory_name']}}</option>
            </select>
        </div>
        <div class="col-md-4">
            <select class="form-control" name="childcategory_id" >
                <option value="">Choisir la sous sous-catégorie</option>
                <option v-for="data in subsubcategories" :value="data.id" :key="data.id">{{data['sub_subcategory_name']}}</option>
            </select>
        </div>
    </div>
</template>
